...
class PCFGANTrainer(BaseTrainer):
    ...
    def fit(self, device):
        ...
        G_loss_copy = torch.tensor(self.losses_history["G_loss"], device = 'cpu')
        plt.plot(D_loss_copy[100:], label='D')
        plt.plot(G_loss_copy[100:], label='G1')
    ...
    def step(self, device, step):
        ...
        for i in range(self.D_steps_per_G_step):
            ...
            with torch.no_grad():
                x_real_batch = next(iter(self.train_dl))[1].to(device)
                x_fake = self.G(
